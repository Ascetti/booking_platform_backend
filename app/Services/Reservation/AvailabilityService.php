...
class AvailabilityService
{
	 ...
	 */
	public function getAvailableCategories(Hotel $hotel, Carbon $checkIn, Carbon $checkOut, int $adults, int $children): Collection
	{
	...
	public function validateBookingData(
		...
		if ($category->hotel_id !== $hotel->id) {
			throw new UnprocessableEntityHttpException(
				'Room category does not belong to this hotel.'
			);
		}
		...
		if (!$category->canAccommodate($adults, $children)) {
			throw new UnprocessableEntityHttpException(
				'Room category cannot accommodate the specified number of guests.'
			);
		}
		...
		if ($roomId !== null) {
			$this->validateRoom($category, $roomId, $checkIn, $checkOut);
		}
	 ...
	 */
	private function validateRoom(RoomCategory $category, int $roomId, Carbon $checkIn, Carbon $checkOut): void
	{
		// Номер должен принадлежать категории и быть активным
		$room = $category->rooms()
			->where('id', $roomId)
			->where('is_active', true)
			->first();
		...
		// Номер должен быть свободен на даты
		$activeStatuses = [
